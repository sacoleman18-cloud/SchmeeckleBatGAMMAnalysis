...
message("\n════════════��══════════════════════════════════════════════════════\n")
# Change reference level to Edge
df_model <- df_model %>%
  mutate(habitat = factor(habitat, levels = c("Edge", "Interior", "Open")))

# Refit model
model_edge_ref <- fit_nb_gamm(df_model, smooth_k = 7, method = "REML", quiet = FALSE)

# Extract new contrasts
habitat_effects_edge_ref <- extract_habitat_effects(model_edge_ref, reference = "Edge")
print(habitat_effects_edge_ref)
